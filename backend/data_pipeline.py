...
def fetch_traffic_and_meteo(timestamp):
    ...
    traffic_df['congestion_index'] = traffic_df['free_flow_speed'] / traffic_df['current_speed']
...
def spatial_temporal_fusion(caaqms_df, sat_df, traffic_df, meteo_data, master_grid):
    ...
    if not sat_df.empty:
        ...
        grid_sat = gpd.sjoin_nearest(master_grid[['grid_id', 'geometry']], sat_gdf, how='left')
        grid_sat = grid_sat.groupby('grid_id').mean(numeric_only=True).reset_index()
    else:
        grid_sat = pd.DataFrame({'grid_id': master_grid['grid_id'], 'sat_no2': np.nan, 'thermal_flag': 0})
    ...
    fused_df = pd.merge(fused_df, grid_sat[['grid_id', 'sat_no2', 'thermal_flag']], on='grid_id', how='left')
    ...
    fused_df['land_use_type'] = np.random.choice(['residential', 'commercial', 'industrial', 'forest'], size=len(fused_df))
...
def feature_engineering(history_df, current_fused_df):
    ...
    # Rolling stats
    df['rolling_mean_6h'] = df.groupby('grid_id')['pm25'].transform(lambda x: x.rolling(window=24, min_periods=1).mean())
    df['rolling_std_6h'] = df.groupby('grid_id')['pm25'].transform(lambda x: x.rolling(window=24, min_periods=1).std())
...
async def run_pipeline():
    ...
    current_fused = spatial_temporal_fusion(caaqms_cleaned, sat_data, traffic_data, meteo_data, master_grid)
    ...
    expected_columns = [
        'timestamp', 'grid_id', 'latitude', 'longitude', 'pm25', 'pm10', 'no2', 
        'traffic_index', 'thermal_flag', 'wind_speed', 'wind_direction', 'pblh', 
        'pm25_lag_1h', 'pm25_lag_3h', 'pm25_lag_24h', 'rolling_mean_6h', 'land_use_type'
    ]
